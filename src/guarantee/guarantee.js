...
define( function(require){
    ...
    var TPL = {
        ...
        guarantee : ''
            + '<ul>'
            +   '<li class="icon-hotel">#{hotel}</li>'
            +   '<li class="icon-glpa">#{glpa}</li>'
            + '</ul>',
    ...
    function onShow(arrow, layer) {
        ...
        // 保障浮层，发送日志 by chengong03
        if( baidu.dom.hasClass(arrow.target, 'icon-guarantee')) {
            sendLog('guarantee');
        }
    }
    ...
    function onBeforeShow(arrow, layer) {
        ...
        // 从dom属性中优惠券浮层数据 by chengong03
        var guaranteeJson = getDataFromAttr(arrowDom, 'data-guarantee');
        ...
        // 构造保障浮层html by chengong03
        if(guaranteeJson) {
            title = guaranteeTitleHtml(guaranteeJson);
            content = guaranteeBodyHtml(guaranteeJson);
    ...
    /**
     * 保障标题html by chengong03
     * 
     * @param {[type]} json 保障数据
     * @return {[type]} 返回拼装好的标题html
     */
    function guaranteeTitleHtml(json) {
        var title = format('title', {title: json.title});
        return title;
    }

    /**
     * 保障正文 html by chengong03
     * 
     * @param {[type]} json 保障数据
     * @return {[type]} 返回拼装好的标题html
     */
    function guaranteeBodyHtml(json) {
        var content = format('guarantee', {
            hotel: json.hotel,
            glpa: json.glpa
        });
    ...
    function init(opts) {
            ...
            baidu.dom.ready(function() {
                ...
                // 引入tip所需的css
                // var url = 'http://s1.bdstatic.com/r/www/cache/biz/ecom/common/api/guarantee0808/guarantee.css';
                var url = 'src/guarantee.css';
